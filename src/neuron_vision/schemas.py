...
class TriageResult(BaseModel):
    ...
    board_type: str = Field(
        description="PCB board type, e.g. 'double-layer SMT', 'single-layer through-hole'"
    )
    risk_zones: list[str] = Field(
        description="Areas requiring close inspection, e.g. ['BGA clusters', 'fine-pitch IC']"
    )
...
class ComponentReport(BaseModel):
    ...
    misoriented: list[str] = Field(
        default_factory=list, description="Component refs with wrong orientation"
    )
    shifted: list[str] = Field(
        default_factory=list, description="Component refs with placement shift"
    )
...
class MarkingIssue(BaseModel):
    ...
    issue_type: Literal[
        "illegible", "missing", "damaged", "incorrect_polarity_mark", "qr_unreadable"
    ]
    severity: Literal["minor", "moderate", "critical"]
    confidence: float = Field(ge=0.0, le=1.0)
...
class MarkingReport(BaseModel):
    ...
    unreadable: list[str] = Field(
        default_factory=list, description="Text areas that are unreadable"
    )
    missing_marks: list[str] = Field(
        default_factory=list, description="Expected marks that are absent"
    )
