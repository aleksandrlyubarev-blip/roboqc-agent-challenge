...
- [x] Версии зависимостей запинены: `infra/fable5/requirements.lock` (используется в Dockerfile)
...
### Запуск, когда появится доступ

```bash
# Всё одной командой: APIs + секрет + SA + Artifact Registry + build + deploy
ANTHROPIC_KEY_VALUE='sk-ant-...' ./scripts/setup_fable5_gcp.sh
# Проверка
./scripts/smoke_fable5.sh
```
