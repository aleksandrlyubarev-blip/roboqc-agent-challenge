...
**Этап 2 — Надёжность:** ✅ выполнен в этой ветке
C4 (project_id через параметр + валидация формата, без мутации `os.environ`),
M4 (regex-парсинг JSON из ответов модели), M8 (`config.py`: `ROBOQC_MODEL` /
`GOOGLE_CLOUD_LOCATION`; `GEMINI_MODEL` / `GOOGLE_CLOUD_REGION` для
neuron_vision), M9 (валидаторы схем: `gs://`-URI, max_length, обязательный
rationale при override, пересчёт и отклонение неверной гистограммы),
M7 (интеграционные тесты пайплайна на фейковых агентах + async demo).
M3 — закрыт проверкой: клиент Fable5 уже разделяет retryable
(транспорт/refusal → fallback) и schema-drift (`ValidationError` → громкий
`Fable5Error` без ретрая); доработка не требовалась.

**Этап 3 — Архитектура:** ✅ выполнен в этой ветке
C6 — `orchestration/board_flow.py`: `BoardFlowCoordinator` (триггер финализации
после всех тайлов + персистентность через execution store) и
`resolve_tile_action` (FrictionPolicyEngine как детерминированный
enforcement-слой над предложением Supervisor-LLM); `output_key` у всех четырёх
агентов (state-ключи `defects`/`fmea_entries`/`action`/`qc_report`).
Полная runtime-инвокация ADK-графа против живого Vertex остаётся за рамками
ветки (нужен GCP-доступ).
M1 — решение зафиксировано в [ADR 0001](adr/0001-two-packages.md):
`neuron_vision` = продуктовый runtime, `roboqc_agent` = ADK
reference-архитектура; слияние доменных схем отклонено осознанно.
M10 — миграция `SequentialAgent` → `google.adk.workflow.Workflow` выполнена.
L11 — метрика `roboqc_fable5_fallback_count` + budget-алерт в
`infra/monitoring/README.md` (требует роль Billing Admin, выполняется вручную).

**Низкоприоритетные:** ✅ L1–L5, L7–L12 закрыты. L6 — широкие `except` снабжены
логированием (полная типизация исключений оставлена как фоновая задача).
Не делалось осознанно: матрица ОС в CI (деплой — только linux/Cloud Run);
порядок stage-колбэков в пайплайне (колбэки обязаны выполняться в основном
потоке Streamlit — текущее поведение корректно по построению).

---

*Финальная проверка в чистом CI-окружении: `pytest` — 72 passed, покрытие 85%
(гейт в CI поднят до 75%), `ruff` / `black` / `mypy src` — без замечаний.*
